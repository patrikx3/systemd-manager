...
# ⌚ SystemD Manager, watchdog, notifier and service v2026.4.100
...
# or use flags / env var
./notifier --settings ./settings.json
P3X_SYSTEMD_SETTINGS=./settings.json ./notifier

# it is used to be a watchdog, polling
./watchdog settings.json
./watchdog --settings ./settings.json
P3X_SYSTEMD_SETTINGS=./settings.json ./watchdog

If installed from npm, the binaries are available as:

```bash
npx p3x-systemd-notifier --settings ./settings.json
npx p3x-systemd-watchdog --settings ./settings.json
```
```
...
[**P3X-SYSTEMD-MANAGER**](https://corifeus.com/systemd-manager) Build v2026.4.100
